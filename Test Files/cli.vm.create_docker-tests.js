  ...
  describe('vm', function() {
    ...
    beforeEach(function(done) {
      suite.setupTest(function() {
        ...
        vmName = suite.isMocked ? 'XplattestVmDock' : suite.generateId(vmPrefix, null);
    ...
    describe('Vm Create: ', function() {
      ...
      it('Create Docker VM with invalid docker cert dir should throw error', function(done) {
        getImageName('Linux', function(ImageName) {
          var cmd = util.format('vm docker create %s %s %s %s --json --ssh 22 --docker-cert-dir F:/foo/bar',
            vmName, ImageName, username, password).split(' ');
          cmd.push('--location');
          cmd.push(location);
          testUtils.executeCommand(suite, retry, cmd, function(result) {
